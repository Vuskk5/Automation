...
public class SelectImpl extends ElementImpl implements Select {
    ...
    public void selectRandom() {
        int size = innerSelect.getOptions().size();

        if (size > 0) {
            selectByIndex((int)(Math.random() * size));
        }
    }

    public void selectRandomRange(int start, int end) {
        int size = innerSelect.getOptions().size();

        if (size > 0) {
            selectByIndex(start + (int)(Math.random() * (size - start - end)));
        }
    }

    public void selectRandomWithOffset(int offset) {
        int size = innerSelect.getOptions().size();

        if (size > 0) {
            selectByIndex(offset + (int)(Math.random() * (size - offset)));
        }
    }
